...
/**
 * Abstract implementation of the basic functions needed for a binary heap using
 * java.util.Vector as a back end.
 *
 * Unlike java.util.TreeSet, this data structure can handle duplicate entries.
 * The implementations of the methods given here implement a MinHeap. It is
 * abstract so that we can have a MinHeap class that supports deleteMin() and a
 * MaxHeap class that supports deleteMax() but neither of them support both.
 *
...
public abstract class Heap<T> extends Vector<T> {
    ...
    /**
     * Remove the Object o from the Heap and return true. Returns false if o is
     * not in the Heap (as measured by o.equals()).
     */
    @Override
    ...
    /**
     * Ensure that every element in this heap obeys the heap property. Runs in
     * linear time.
     *
     * This is meant to be called if/when the Comparator associated with this
     * object is modified.
     */
    ...
    /**
     * Perform an in place heap sort on the data stored in this heap. After
     * calling sort, a call to this objects iterator() method will iterate
     * through the data stored in the heap in ascending sorted order. This is
     * not a stable sort.
     */
    public void sort() {
        Object[] a = toArray();
    ...
    /**
     * Ensure that the subtree of the given size rooted at node obeys the heap
     * property
     */
