 ...
 * @author Koen Samyn
 */
public class InputConnectorPanel extends javax.swing.JPanel {

    private Rig rig;
    private AnimationController currentAnimationController;
    private InputConnector currentInputConnector;
    private String currentPanelId;

    /**
     * Creates new form InputConnectorPanel
     */
    public InputConnectorPanel() {
        initComponents();
    }
    ...
    private void updateCustomizerPanel(ConnectorType ict) {
        ...
        try {
            ...
            InputCustomizer panel = (InputCustomizer) panelClass.newInstance();
            panel.setRig(rig);
    ...
    /**
     * Creates a new input connector based on the current settings of the panel.
     *
     * @return a new input connector.
     */
    public InputConnector createNewInputConnector() {
        for (Component c : pnlConnectorTypes.getComponents()) {
            if (c.isVisible() && c instanceof InputCustomizer) {
                InputCustomizer ic = (InputCustomizer) c;
                return ic.createConnector();
            }
        }
        return null;
    }

    private void setInputConnectorOnCustomizerPanel() {
        for (Component c : pnlConnectorTypes.getComponents()) {
            if (c.isVisible() && c instanceof InputCustomizer) {
                InputCustomizer ic = (InputCustomizer) c;
                ic.setInputConnector(currentInputConnector);
            }
        }
    }
